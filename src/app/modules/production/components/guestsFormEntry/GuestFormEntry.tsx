...
const GuestMultiTabForm = () => {
  ...
  const OnSubmit = handleSubmit(async (values, event) => {
    if(values.firstName===''||values.lastName===''||values.email==''||values.gender==''||values.dob==''||values.phoneNumber==''||values.idType==''||values.nationality==''||values.idNumber==''){
      message.error('Sorry, all fields must be filled before proceeding.')
      setLoading(false)
      return
    }
    event?.preventDefault()
  ...
  message.error('Sorry this operation failed. Pelase try again.')
  console.log(error)
      setLoading(false)
      return error.statusText
    }
